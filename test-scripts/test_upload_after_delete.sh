...
usage() {
    ...
    echo ""
    echo "This test verifies that uploading AFTER deleting an asset does NOT:"
    echo "  - Move file to asset/ directory"
    echo "  - Create new asset record"
    echo "  - Add asset to section"
    echo "  - Decrement remainingAssets again"
    echo ""
    echo "Example:"
...
echo "Running upload-after-delete test in Docker container..."
...
docker run --rm \
    -v "$(pwd)/test-scripts/test_upload_after_delete.py:/test_upload_after_delete.py" \
    ...
    python:3.11-slim \
    bash -c "pip install boto3 requests && python -u /test_upload_after_delete.py"
