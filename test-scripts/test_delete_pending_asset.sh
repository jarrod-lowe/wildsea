...
usage() {
    ...
    echo ""
    echo "This test verifies that deleting a PENDING asset (never uploaded to S3) works correctly."
    echo ""
    echo "Example:"
...
echo "Running deleteAsset PENDING test in Docker container..."
...
docker run --rm \
    -v "$(pwd)/test-scripts/test_delete_pending_asset.py:/test_delete_pending_asset.py" \
    ...
    python:3.11-slim \
    bash -c "pip install boto3 requests && python -u /test_delete_pending_asset.py"
