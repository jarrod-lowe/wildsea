...
usage() {
    echo "Usage: $0 <username> <password> <game_id> <section_id> [upload_delay_seconds]"
    echo ""
    echo "Arguments:"
    ...
    echo "  upload_delay_seconds Optional delay in seconds between requestAssetUpload and S3 upload (default: 0)"
    echo ""
    echo "This script tests the complete asset upload and event-driven finalization flow:"
    echo "1. requestAssetUpload mutation"
    echo "2. Optional delay (useful for testing upload URL expiration)"
    echo "3. S3 file upload using presigned URL"
    echo "4. S3 event → EventBridge → _finaliseAsset resolver"
    echo "5. Asset status change from PENDING to READY"
    echo ""
    echo "Examples:"
    echo "  $0 username 'mypassword' 'cd69661a-57e4-450d-8670-1058958b1bfe' '9187e56a-07f2-4ca8-945a-264ad970e2e2'"
    echo "  $0 username 'mypassword' 'cd69661a-57e4-450d-8670-1058958b1bfe' '9187e56a-07f2-4ca8-945a-264ad970e2e2' 30"
    exit 1
}
...
GAME_ID="$3"
SECTION_ID="$4"
UPLOAD_DELAY="${5:-0}"  # Default to 0 if not provided
...
echo "Running complete asset upload and finalization test in Docker container..."
echo "GraphQL URL: $GRAPHQL_URL"
echo "Game ID: $GAME_ID"
echo "Section ID: $SECTION_ID"
echo "Username: $COGNITO_USERNAME"
echo "Upload Delay: $UPLOAD_DELAY seconds"
echo ""

# Build docker run command with required environment variables
docker run --rm \
    -v "$(pwd)/test-scripts/test_complete_asset_flow.py:/test_complete_asset_flow.py" \
    ...
    -e UPLOAD_DELAY="$UPLOAD_DELAY" \
    python:3.11-slim \
    bash -c "pip install requests && python -u /test_complete_asset_flow.py"